...
当前默认线上入口是产品模式。Vercel 打开 `/` 会直接进入 `frontend/product.html`。
...
- `frontend/index.html` 是工程测试页，用于手动输入/上传图片并测试 AI 小票生成。
- 测试页每次步骤完成或失败会显示当前时间。
...
frontend/
  index.html              # 工程测试页
  product.html            # 移动端产品模式，Vercel 默认入口
  debug.html              # 调试面板
  styles.css
  src/app.ts              # 测试页交互
...
3. 运行 `npm run dev`。
4. 打开 `http://localhost:5173/index.html`。
...
Vercel 根路径 `/` 会 rewrite 到 `/product.html`。
